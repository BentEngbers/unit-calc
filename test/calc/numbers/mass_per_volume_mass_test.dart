...
void main() {
  group('AmountPerMLKG', () {
    ...
    test("bad json", () {
      expect(() => MassPerVolumeMass.fromJson("4.2 mg/ml/kg/hr"),
    ...
    for (final testCase in testCases) {
      ...
      test("json round trip", () {
        expect(MassPerVolumeMass.fromJson(massPerMlKg.toJson()),
            equals(massPerMlKg));
      });

      test("mass multiplication", () {
        expect(
            massPerMlKg * Mass(mass, U(factorToNg: 1)),
            equals(
      ...
      for (final previousElement in previousElements) {
        final previousMassPerMlKg = MassPerVolumeMass.perKg(
            previousElement.amountPerMlKg, U(factorToNg: 1), VolumeUnit.ml);
        ...
        test("Check $previousMassPerMlKg > $massPerMlKg ", () {
          expect(previousMassPerMlKg.hashCode,
